...
pub fn merge_job_pool(
    our_payout_script: &Option<Script>,
    work: &WorkProviderJob,
    pool: Option<&PoolProviderJob>,
    user: Option<&PoolProviderUserJob>,
) -> Option<WorkInfo> {
    ...
    for output in template.appended_coinbase_outputs.iter() {
        if output.value != 0 {
            panic!("We should have checked this on the recv end!");
        }
    }
    ...
    match &work.coinbase_prefix_postfix {
        &Some(ref postfix) => {
            template
                .coinbase_prefix
                .extend_from_slice(&postfix.coinbase_prefix_postfix[..]);
        }
        &None => {}
    }
    ...
    match pool {
        Some(&PoolProviderJob {
            ...
            let value_remaining =
                (template.coinbase_value_remaining as i64) - (constant_value_output as i64);
            if value_remaining <= 0 {
                println!(
                    "Pool requiring {} in output value, work provider only finding {}! Can't mine!",
                    constant_value_output, template.coinbase_value_remaining
                );
                return None;
            }
            ...
            match user {
                Some(&PoolProviderUserJob {
                    ref coinbase_postfix,
                    ref target,
                }) => {
                    ...
                    template
                        .coinbase_postfix
                        .extend_from_slice(coinbase_postfix);
                }
                None => {}
            }
        }
    ...
    tokio::spawn(
        solution_rx
            .for_each(move |nonces: Arc<(WinningNonce, Sha256dHash)>| {
