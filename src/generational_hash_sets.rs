...
impl GenerationalHashSets {
    ...
    pub fn try_insert(&self, generation: &[u8; 32], value: Sha256dHash) -> bool {
        ...
        let res = if same_generation {
            let set_ref: &mut HashSet<Sha256dHash> =
                unsafe { &mut *(*self.latest_set.get()).1.get() };
            ...
            self.storage
                .lock()
                .unwrap()
                .insert(generation.clone(), set_arc.clone());
            let gen_ref: &mut ([u8; 32], Arc<UnsafeObj<HashSet<Sha256dHash>>>) =
                unsafe { &mut *self.latest_set.get() };
