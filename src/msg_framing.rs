...
impl BlockTemplate {
    pub fn encode_unsigned(&self, res: &mut bytes::BytesMut) {
        ...
        le16_into_slice(
            (res.len() - remaining_len_pos) as u16,
            &mut res[remaining_len_pos - 2..remaining_len_pos],
        );
    }
}
    ...
    fn encode(&mut self, msg: WorkMessage, res: &mut bytes::BytesMut) -> Result<(), io::Error> {
        match msg {
            ...
            WorkMessage::VendorMessage {
                ...
                let len =
                    1 + if signature.is_some() { 64 } else { 0 } + 1 + vendor.len() + message.len();
                if len > 0xffffff {
                    return Err(io::Error::new(io::ErrorKind::InvalidData, CodecError));
                }
    ...
    fn decode(&mut self, bytes: &mut bytes::BytesMut) -> Result<Option<WorkMessage>, io::Error> {
        ...
        match bytes[0] {
            ...
            4 => {
                ...
                let coinbase_sketch: Transaction =
                    match network::serialize::deserialize(&coinbase_sketch_data[..]) {
                        Ok(tx) => tx,
    ...
    fn encode(&mut self, msg: PoolMessage, res: &mut bytes::BytesMut) -> Result<(), io::Error> {
        match msg {
            ...
            PoolMessage::Share { ref share } => {
                ...
                let header_len = if share.previous_header.is_some() {
                    80
                } else {
                    0
                };
                res.reserve(
                    1 + 3
                        ...
                        + 32
                        + 1
                        + share.merkle_rhss.len() * 32
                        + 4
                        + tx_enc.len()
                        + 1
                        + share.user_tag_1.len()
                        + 1
                        + share.user_tag_2.len()
                        + header_len,
                );
                res.put_u8(19);
