...
pub fn share_submitted(
    state: &KafkaSubmitterState,
    user_id: &Vec<u8>,
    user_tag_1: &Vec<u8>,
    value: u64,
    header: &BlockHeader,
    leading_zeros: u8,
    required_leading_zeros: u8,
) {
    println!(
        "Got valid share with value {} from \"{}\" from machine identified as \"{}\"",
        value,
        String::from_utf8_lossy(user_id),
        String::from_utf8_lossy(user_tag_1)
    );
    ...
    tokio::spawn(
        state
            .kafka_producer
            .send(
                FutureRecord::to(&state.topic).key("").payload(
...
pub fn weak_block_submitted(
    ...
    tokio::spawn(
        state
            ...
            .send(
                FutureRecord::to(&state.topic).key("").payload(
                    &serde_json::to_string(&WeakBlockMessage {
                        user: String::from_utf8_lossy(&user_id).to_string(),
                        worker: String::from_utf8_lossy(&user_tag_1).to_string(),
                        payout: value,
                        client_target: required_leading_zeros,
