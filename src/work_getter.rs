    ...
    pub fn create(mut pool_hosts: Vec<PoolInfo>) -> mpsc::UnboundedReceiver<PoolProviderUserWork> {
        ...
        tokio::spawn(future::lazy(move || -> Result<(), ()> {
            for (idx, pool) in pool_hosts.drain(..).enumerate() {
                ...
                auth_write
                    .start_send(PoolAuthAction::AuthUser(PoolUserAuth {
						...
						() => {
                            ...
                            match job {
                                PoolProviderAction::UserUpdate { update, .. } => {
                                    ...
                                    if cur_work.cur_pool >= idx
                                        && cur_work.pools[idx].last_job.is_some()
                                    {
                                        cur_work.cur_pool = idx;
                                        let payout_info =
                                            cur_work.pools[idx].last_job.as_ref().unwrap().clone();
                                        cur_work
                                            .job_tx
                                            .start_send(PoolProviderUserWork {
                                                payout_info,
                                ...
                                PoolProviderAction::PoolUpdate { info } => {
                                    cur_work.pools[idx].is_connected = true;
                                    if cur_work.cur_pool >= idx
                                        && cur_work.pools[idx].last_user_job.is_some()
                                    {
                                        cur_work.cur_pool = idx;
                                        let user_payout_info = cur_work.pools[idx]
                                            .last_user_job
                                            .as_ref()
                                            .unwrap()
                                            .clone();
...
impl WorkGetter {
    pub fn create(
        ...
        pool_server: Vec<PoolInfo>,
        solo_payout_script: Script,
    ) -> mpsc::UnboundedReceiver<WorkInfo> {
        ...
        tokio::spawn(
            MultiJobProvider::create(job_provider_hosts).for_each(move |work_update| {
                let mut cur_work = job_work_rc.lock().unwrap();
                cur_work.cur_work = Some(work_update);
                let cur_pool = if let &Some(ref pool) = &cur_work.cur_pool {
                    Some(&pool.payout_info)
                } else {
                    None
                };
                let cur_user = if let &Some(ref user) = &cur_work.cur_pool {
                    Some(&user.user_payout_info)
                } else {
                    None
                };
                if let Some(work) = merge_job_pool(
                    &cur_work.payout_script,
                    cur_work.cur_work.as_ref().unwrap(),
                    cur_pool,
                    cur_user,
                ) {
        ...
        tokio::spawn(
            MultiPoolProvider::create(pool_server).for_each(move |pool_update| {
                let mut cur_work = cur_work_rc.lock().unwrap();
                if let Some(ref work) = cur_work.cur_work {
                    if let Some(work) = merge_job_pool(
                        &cur_work.payout_script,
                        work,
                        Some(&pool_update.payout_info),
                        Some(&pool_update.user_payout_info),
                    ) {
