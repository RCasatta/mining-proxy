...
pub trait ConnectionHandler<MessageType> {
    ...
    type Framer: codec::Encoder<Item = MessageType, Error = io::Error>
        + codec::Decoder<Item = MessageType, Error = io::Error>
        + Send;
...
pub struct ConnectionMaintainer<
    MessageType: 'static + Send,
    HandlerProvider: ConnectionHandler<MessageType>,
> {
    host: String,
...
impl<
        ...
        HandlerProvider: 'static + ConnectionHandler<MessageType> + Send + Sync,
    > ConnectionMaintainer<MessageType, HandlerProvider>
{
    pub fn new(
        host: String,
        handler: HandlerProvider,
    ...
    pub fn make_connection(mut self) {
        if {
            ...
            tokio::spawn(
                timer::Delay::new(Instant::now() + Duration::from_secs(10)).then(
                    move |_| -> future::FutureResult<(), ()> {
        ...
        match addr_option {
            Some(addr) => {
                ...
                tokio::spawn(net::TcpStream::connect(&addr).then(
                    move |res| -> future::FutureResult<(), ()> {
                        match res {
                            Ok(stream) => {
            ...
            None => {
                tokio::spawn(
                    timer::Delay::new(Instant::now() + Duration::from_secs(10)).then(move |_| {
                        self.make_connection();
                        future::result(Ok(()))
                    }),
                );
