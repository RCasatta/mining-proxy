...
impl EventualTxData {
    fn new() -> (Arc<Self>, oneshot::Sender<TransactionData>) {
        ...
        let us_rx = us.clone();
        tokio::spawn(
            rx.and_then(move |res: TransactionData| {
                *us_rx.value.write().unwrap() =
                    Some((res.transactions, res.previous_header, res.extra_block_data));
...
impl JobProviderHandler {
    fn new(
        expected_auth_key: Option<PublicKey>,
    ) -> (Arc<JobProviderHandler>, mpsc::Receiver<WorkProviderAction>) {
    ...
    pub fn send_nonce(&self, work: WinningNonce) {
        ...
        match &state.stream {
            &Some(ref stream) => {
                match stream.unbounded_send(WorkMessage::WinningNonce { nonces: work }) {
                    Ok(_) => {
                        println!("Submitted job-matching (ie full-block) nonce!");
                    }
                    Err(_) => {
                        println!("Failed to submit job-matching (ie full-block) nonce as job provider disconnected");
                    }
                }
            }
            &None => {
...
impl ConnectionHandler<WorkMessage> for Arc<JobProviderHandler> {
    ...
    fn new_connection(&self) -> (WorkMsgFramer, mpsc::UnboundedReceiver<WorkMessage>) {
        ...
        match tx.start_send(WorkMessage::ProtocolSupport {
            ...
            Err(_) => {
                println!("Job Provider disconnected before we could send version handshake");
            }
        }
    ...
    fn connection_closed(&self) {
        ...
        let _ = us
            .job_stream
            .start_send(WorkProviderAction::ProviderDisconnected);
        us.stream = None;
    }

    fn handle_message(&self, msg: WorkMessage) -> Result<(), io::Error> {
        let mut us = self.state.lock().unwrap();
        if us.stream.is_none() {
            return Ok(());
        }
        ...
        match msg {
            ...
            WorkMessage::ProtocolVersion {
                ...
                println!(
                    "Received ProtocolVersion, using version {}",
                    selected_version
                );
            }
            ...
            WorkMessage::BlockTemplate {
                ...
                if template.template_timestamp < timestamp - 1000 * 60 * 20
                    || template.template_timestamp > timestamp + 1000 * 60 * 1
                {
                    println!(
                        "Got template with unreasonable timestamp ({}, our time is {})",
                        template.template_timestamp, timestamp
                    );
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        utils::HandleError,
                ...
                match us.cur_prefix_postfix {
                    Some(ref prefix_postfix) => {
                        if prefix_postfix.coinbase_prefix_postfix.len()
                ...
                if us.cur_template.is_none()
                    || us.cur_template.as_ref().unwrap().template_timestamp
                        < template.template_timestamp
                {
                    println!(
                        "Received new BlockTemplate with diff lower bound {}",
                        utils::target_to_diff_lb(&template.target)
                    );
                    let (txn, txn_tx) = EventualTxData::new();
                    match us.stream.as_ref().unwrap().unbounded_send(
                        WorkMessage::TransactionDataRequest {
                            template_timestamp: template.template_timestamp,
                        },
                    ) {
                        Ok(_) => {}
                    ...
                    us.pending_tx_data_requests
                        .insert(template.template_timestamp, txn_tx);
            ...
            WorkMessage::TransactionData { signature, data } => {
                ...
                match us.pending_tx_data_requests.remove(&data.template_timestamp) {
                    Some(chan) => match chan.send(data) {
                        Ok(()) => {}
                        Err(_) => {
            ...
            WorkMessage::CoinbasePrefixPostfix {
                ...
                let time = SystemTime::now().duration_since(UNIX_EPOCH).unwrap();
                let timestamp = time.as_secs() * 1000 + time.subsec_nanos() as u64 / 1_000_000;
                if coinbase_prefix_postfix.timestamp < timestamp - 1000 * 60 * 20
                    || coinbase_prefix_postfix.timestamp > timestamp + 1000 * 60 * 1
                {
                ...
                match us.cur_template {
                    Some(ref template) => {
                        if coinbase_prefix_postfix.coinbase_prefix_postfix.len()
                            + template.coinbase_prefix.len()
                            > 42
                        {
                ...
                if us.cur_prefix_postfix.is_none()
                    || us.cur_prefix_postfix.as_ref().unwrap().timestamp
                        < coinbase_prefix_postfix.timestamp
                {
                            ...
                            match job {
                                ...
                                WorkProviderAction::ProviderDisconnected => {
                                    if cur_work.jobs[idx].is_connected {
                                        ...
                                        if cur_work.jobs[idx].last_job.is_some()
                                            && cur_work.jobs[idx]
                                                .last_job
                                                .as_ref()
                                                .unwrap()
                                                ...
                                                == cur_work.best_job
                                        {
                                            ...
                                            for (idx, job) in cur_work.jobs.iter().enumerate() {
                                                if job.last_job.is_some() {
                                                    let job_template =
                                                        job.last_job.as_ref().unwrap();
                                                    if job_template.tx_data.has_result()
                                                        && job_template.template.template_timestamp
                                                            > highest_timestamp.0
                                                    {
                                                        highest_timestamp = (
                                                            job_template
                                                    ...
                                                    if job.is_connected
                                                        && job_template.template.template_timestamp
                                                            > highest_connected_timestamp.0
                                                    {
                                                        highest_connected_timestamp = (
                                                            job_template
                                                                .template
                                                                .template_timestamp,
                                                            idx,
                                            ...
                                            if highest_connected_timestamp.0 != 0 {
                                                let new_job = cur_work.jobs
                                                    [highest_connected_timestamp.1]
                                                    .last_job
                                                    .as_ref()
                                                    .unwrap()
                                                    .clone();
                                                ...
                                                let new_job = cur_work.jobs[highest_timestamp.1]
                                                    .last_job
                                                    .as_ref()
                                                    .unwrap()
                                                    .clone();
