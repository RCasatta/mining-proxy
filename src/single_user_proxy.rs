...
fn main() {
    ...
    rt.spawn(future::lazy(move || -> Result<(), ()> {
        let job_rx = WorkGetter::create(
            job_provider_hosts,
            pools,
            payout_addr.clone().unwrap().script_pubkey(),
        );
        ...
        if stratum_listen_bind.is_some() && mining_listen_bind.is_none() {
            bind_and_handle!(
                stratum_listen_bind,
                StratumServer::new(job_rx, None),
                StratumServer
            );
        } else if stratum_listen_bind.is_none() && mining_listen_bind.is_some() {
            bind_and_handle!(
                mining_listen_bind,
                MiningServer::new(job_rx, mining_auth_key.unwrap()),
                MiningServer
            );
        } else {
            ...
            bind_and_handle!(
                stratum_listen_bind,
                StratumServer::new(stratum_rx, None),
                StratumServer
            );
            bind_and_handle!(
                mining_listen_bind,
                MiningServer::new(mining_rx, mining_auth_key.unwrap()),
                MiningServer
            );
        }
