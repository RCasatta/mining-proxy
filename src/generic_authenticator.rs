...
pub fn parse_authenticator_parameter(
    _settings: &mut GenericAuthenticatorSettings,
    _arg: &str,
) -> bool {
...
pub fn check_user_auth(
    _state: &GenericAuthenticatorState,
    user_id: &Vec<u8>,
    user_auth: &Vec<u8>,
) -> bool {
    println!(
        "User {} authed with pass {}",
        String::from_utf8_lossy(user_id),
        String::from_utf8_lossy(user_auth)
    );
    true
}
