...
impl PoolHandler {
    pub fn new(
        ...
        user_auth_requests: mpsc::Receiver<PoolAuthAction>,
    ) -> (Arc<PoolHandler>, mpsc::Receiver<PoolProviderAction>) {
            ...
            match auth_action {
                PoolAuthAction::AuthUser(user_id_auth) => {
                    ...
                    if let &mut Some(ref mut stream) = refs.stream {
                        let _ = stream.start_send(PoolMessage::UserAuth {
                            info: user_id_auth.clone(),
                        });
                    }
                ...
                PoolAuthAction::DropUser(user_id) => {
                    let (timestamp, postfix, to_reauth_posn) =
                        refs.user_id_to_postfix.remove(&user_id).unwrap();
                    if timestamp != 0 {
                        refs.coinbase_postfix_to_difficulty
                            .remove(&postfix)
                            .unwrap();
                    }
                    refs.users_to_reauth.swap_remove(to_reauth_posn);
                    if refs.users_to_reauth.len() > to_reauth_posn {
                        refs.user_id_to_postfix
                            .get_mut(&refs.users_to_reauth[to_reauth_posn].user_id)
                            .unwrap()
                            .2 = to_reauth_posn;
                    }
                    if let &mut Some(ref mut stream) = refs.stream {
    ...
    pub fn send_nonce(
        &self,
        work: &(WinningNonce, Sha256dHash),
        template: &Arc<BlockTemplate>,
        post_coinbase_txn: &Vec<Vec<u8>>,
        prev_header: &BlockHeader,
        extra_block_data: &Vec<u8>,
    ) {
        ...
        if let Some(coinbase_postfix_match_len) = *us.coinbase_postfix_len {
            let coinbase_postfix = if work.0.coinbase_tx.input.len() == 1 {
                ...
                if coinbase.len() < coinbase_postfix_match_len as usize {
                    return;
                }
            ...
            if let Some(ref difficulty) = us.coinbase_postfix_to_difficulty.get(&coinbase_postfix) {
                if utils::does_hash_meet_target(&work.1[..], &difficulty.share_target[..]) {
                    let previous_header = if *us.last_header_sent == template.header_prevblock {
                        None
                    } else {
                ...
                if utils::does_hash_meet_target(&work.1[..], &difficulty.weak_block_target[..]) {
                    match us.stream {
                        &mut Some(ref stream) => {
                            ...
                            actions.push(WeakBlockAction::NewTx {
                                tx: network::serialize::serialize(&work.0.coinbase_tx).unwrap(),
                            });

                            match us.last_weak_block.take() {
                                Some(mut last_weak_block) => {
                                    let mut old_txids_posn =
                                        HashMap::with_capacity(last_weak_block.len());
                                    ...
                                    for tx in post_coinbase_txn {
                                        match old_txids_posn.get(tx) {
                                            None => actions
                                                .push(WeakBlockAction::NewTx { tx: tx.clone() }),
                                            Some(&idx) => actions
                                                .push(WeakBlockAction::TakeTx { n: idx as u16 }),
                                        }
                                    }
                                }
                                None => {
...
impl ConnectionHandler<PoolMessage> for Arc<PoolHandler> {
    ...
    fn new_connection(&self) -> (PoolMsgFramer, mpsc::UnboundedReceiver<PoolMessage>) {
        ...
        match tx.start_send(PoolMessage::ProtocolSupport {
            ...
            Err(_) => {
                println!("Pool disconnected before we could send version handshake");
            }
        }
    ...
    fn connection_closed(&self) {
        ...
        let _ = us
            .job_stream
            .start_send(PoolProviderAction::ProviderDisconnected);
    }
    ...
    fn handle_message(&self, msg: PoolMessage) -> Result<(), io::Error> {
        ...
        macro_rules! check_msg_timestamp {
            ($msg: expr, $msg_type_str: expr) => {
                ...
                if $msg.timestamp < timestamp - 1000 * 60 * 20
                    || $msg.timestamp > timestamp + 1000 * 60 * 1
                {
                    println!(
                        "Got {} with unreasonable timestamp ({}, our time is {})",
                        $msg_type_str, $msg.timestamp, timestamp
                    );
                    return Err(io::Error::new(
                        io::ErrorKind::InvalidData,
                        utils::HandleError,
        ...
        match msg {
            ...
            PoolMessage::ProtocolVersion {
                ...
                for user_auth in us.users_to_reauth.iter() {
                    match us
                        ...
                        .as_ref()
                        .unwrap()
                        .start_send(PoolMessage::UserAuth {
                            info: user_auth.clone(),
                        }) {
                        Ok(_) => {}
                        Err(_) => {
                            return Err(io::Error::new(
            ...
            PoolMessage::PayoutInfo {
                ...
                if us.cur_payout_info.is_none()
                    || us.cur_payout_info.as_ref().unwrap().timestamp < payout_info.timestamp
                {
                        ...
                        if *last_timestamp < info.timestamp {
                            ...
                            if let &Some(ref difficulty) = &cur_difficulty {
                                match refs.job_stream.start_send(PoolProviderAction::UserUpdate {
                                    ...
                                    update: PoolProviderUserJob {
                                        ...
                                        target: utils::max_le(
                                            difficulty.share_target.clone(),
                                            difficulty.weak_block_target.clone(),
                                        ),
                                    },
                                }) {
                                    Ok(_) => {}
                            ...
                            *last_postfix = info.coinbase_postfix.clone();
                            if let Some(difficulty) = cur_difficulty {
                                refs.coinbase_postfix_to_difficulty
                                    .insert(info.coinbase_postfix, difficulty);
                            }
                        }
                    }
                    None => {
            ...
            PoolMessage::RejectUserAuth { user_id } => {
                ...
                match us
                    .job_stream
                    .start_send(PoolProviderAction::UserReject { user_id })
                {
                    Ok(_) => {}
                    Err(_) => {
            ...
            PoolMessage::ShareDifficulty { difficulty } => {
                ...
                match refs.user_id_to_postfix.get(&difficulty.user_id) {
                    Some(&(ref last_timestamp, ref last_postfix, _)) => {
                        if *last_timestamp != 0 {
                            let entry = refs
                                .coinbase_postfix_to_difficulty
                                .entry(last_postfix.clone());
                            match entry {
                                hash_map::Entry::Occupied(ref e) => {
                                    if e.get().timestamp >= difficulty.timestamp {
                                        return Ok(());
                                    }
                                }
            ...
            PoolMessage::ShareRejected { reason, .. } => {
                match reason {
                    ShareRejectedReason::StalePrevBlock => {
                        println!("Share REJECTED (stale prev block)!")
                    }
                    ...
                    ShareRejectedReason::BadPayoutInfo => {
                        println!("Share REJECTED (bad payout info)!")
                    }
