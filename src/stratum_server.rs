use msg_framing::{BlockTemplate, PoolUserAuth, WinningNonce};
...
fn template_timestamp_to_job_id(template_timestamp: u64) -> u32 {
    ((template_timestamp & 0xffffffff00000000) >> 32) as u32
        ^ (template_timestamp & 0xffffffff) as u32
}

const EXTRANONCE2_SIZE: usize = 8;
const VERSION_MASK: u32 = 0x1fffe000;
fn job_to_json_string_prefix(
    template: &BlockTemplate,
    update_id: u32,
    user_coinbase_postfix_len: usize,
) -> String {
    let mut prefix = String::with_capacity(197 + template.coinbase_prefix.len() * 2);
    prefix.push_str("{\"params\":[\""); // 12 chars
    let job_id = (template_timestamp_to_job_id(template.template_timestamp) as u64) << 32
        | (update_id as u64);
    ...
    let coinbase_len = template.coinbase_prefix.len()
        + 12
        + EXTRANONCE2_SIZE
        + template.coinbase_postfix.len()
        + user_coinbase_postfix_len;
...
fn job_to_json_string_postfix(template: &BlockTemplate, prev_changed: bool) -> String {
    let mut postfix = String::with_capacity(
        57 + template.coinbase_postfix.len() * 2
            + 64 * template.appended_coinbase_outputs.len()
            + 66 * template.merkle_rhss.len(),
    );
    ...
    postfix.push(
        char::from_digit(
            ((template.appended_coinbase_outputs.len() >> 4) & 0x0f) as u32,
            16,
        )
        .unwrap(),
    ); // 1 char
    postfix.push(
        char::from_digit(
            ((template.appended_coinbase_outputs.len() >> 0) & 0x0f) as u32,
            16,
        )
        .unwrap(),
    ); // 1 char
    for output in template.appended_coinbase_outputs.iter() {
        // ~62 chars per entry (rounded up to 64)
    ...
    for (idx, rhs) in template.merkle_rhss.iter().enumerate() {
        // 66 chars per entry + 1
        postfix.push('"');
...
pub enum UserUpdate {
    ...
    // We never construct this in single_user_pool
    #[allow(dead_code)]
    DropUser { user_id: Vec<u8> },
}

impl StratumServer {
    /// If user_providers is set, job_providers' difficulty is ignored
    pub fn new(
        job_providers: mpsc::UnboundedReceiver<WorkInfo>,
        user_providers: Option<(
            mpsc::UnboundedReceiver<UserUpdate>,
            mpsc::Sender<PoolAuthAction>,
        )>,
    ) -> Arc<Self> {
        let (user_job_stream, user_auth_requests) =
            if let Some((user_job_stream, auth_sink)) = user_providers {
                (Some(user_job_stream), Some(Mutex::new(auth_sink)))
            } else {
                (None, None)
            };
        ...
        tokio::spawn(job_providers.for_each(move |job| {
            {
                ...
                jobs.insert(
                    template_timestamp_to_job_id(job.template.template_timestamp),
                    new_job,
                );
            }
            ...
            let user_coinbase_postfix_len = if need_work_diff {
                0
            } else {
                us_cp.user_coinbase_postfix_len.load(Ordering::Acquire)
            };
            let job_update_id =
                (us_cp.job_update_id.fetch_add(1, Ordering::AcqRel) & 0xffffffff) as u32;
            let job_json_prefix =
                job_to_json_string_prefix(&job.template, job_update_id, user_coinbase_postfix_len);
            ...
            if need_work_diff {
                ...
                let users: Vec<StratumUser> = us_cp
                    .users
                    .lock()
                    .unwrap()
                    .values()
                ...
                for user in users {
                    if let &Some(ref job) = &user.cur_job {
                        let job_json = job_json_prefix.clone()
                            + &utils::bytes_to_hex(&job.coinbase_postfix)
                            + &job_json_postfix;
            ...
            tokio::spawn(users.for_each(move |user_update| {
                match user_update {
                    UserUpdate::WorkUpdate { user_id, user_info } => {
                        us_cp
                            .user_coinbase_postfix_len
                            .store(user_info.coinbase_postfix.len(), Ordering::Release);
                        let (need_postfix_update, need_diff_update, clients) = {
                            ...
                            if let Some(user) = users.get_mut(&user_id) {
                                let postfix_update = user.cur_job.is_none()
                                    || user.cur_job.as_ref().unwrap().coinbase_postfix
                                        != user_info.coinbase_postfix;
                        ...
                        if need_diff_update || need_postfix_update {
                            let last_job = {
                                ...
                                if let Some((_, v)) = jobs.iter().last() {
                                    //TODO: This is ineffecient, map should have a last()
                                    v.clone()
                                } else {
                                    return Ok(());
                                }
                            ...
                            let now = Instant::now();
                            let job_update_id = (us_cp.job_update_id.fetch_add(1, Ordering::AcqRel)
                                & 0xffffffff)
                                as u32;
                            let job_prefix = job_to_json_string_prefix(
                                &last_job.template,
                                job_update_id,
                                user_info.coinbase_postfix.len(),
                            );
                            let job_postfix = job_to_json_string_postfix(&last_job.template, true);
                            let job_string = job_prefix
                                + &utils::bytes_to_hex(&user_info.coinbase_postfix)
                                + &job_postfix;
                            ...
                            for client in clients.iter() {
                                if client.mining.load(Ordering::Acquire) {
                                    ...
                                    *client.cur_coinbase_postfix.lock().unwrap() =
                                        user_info.coinbase_postfix.clone();
                                }
                            }
                        }
                    }
        ...
        tokio::spawn(
            ...
            .for_each(move |_| {
                let last_job = {
                    ...
                    let timestamp =
                        (time.as_secs() - 30) * 1000 + time.subsec_nanos() as u64 / 1_000_000;

                    let jobs = us_timer.jobs.read().unwrap();
                    let last_job = match jobs.iter().last() {
                        //TODO: This is ineffecient, map should have a last()
                    ...
                    if {
                        // Avoid write lock unless we need it
                        let res = {
                            if jobs.len() > 1 {
                        ...
                        while jobs.len() > 1 {
                            ...
                            let (first_id, second_timestamp) = {
                                ...
                                (
                                    *iter.next().unwrap().0,
                                    iter.next().unwrap().1.template.template_timestamp,
                                )
                            };
                ...
                match last_job {
                    Some(job) => {
                        ...
                        let job_update_id = (us_timer.job_update_id.fetch_add(1, Ordering::AcqRel)
                            & 0xffffffff) as u32;
                        let job_json_prefix = job_to_json_string_prefix(
                            &job.template,
                            job_update_id,
                            us_timer.user_coinbase_postfix_len.load(Ordering::Acquire),
                        );
                        ...
                        if need_work_diff {
                            ...
                            for client in clients {
                                if client.mining.load(Ordering::Acquire)
                                    && *client.last_send.lock().unwrap() < send_target
                                {
                                    client.attempt_send(job_json.clone());
                                    *client.last_send.lock().unwrap() = Instant::now();
                                }
                            ...
                            let users: Vec<StratumUser> = us_timer
                                .users
                                .lock()
                                .unwrap()
                                .values()
    ...
    pub fn new_connection(us: Arc<Self>, stream: net::TcpStream) {
        ...
        let client = {
            ...
            tokio::spawn(
                tx.send_all(send_stream.map_err(|_| -> io::Error {
                    unreachable!();
                }))
                .then(|_| future::result(Ok(()))),
            );
